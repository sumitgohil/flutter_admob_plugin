...
class FlutterAdmobPlugin(private val context: Context): MethodCallHandler {
  companion object {
    @JvmStatic
    fun registerWith(registrar: Registrar) {
      val defaultChannel = MethodChannel(registrar.messenger(), "admob")
      defaultChannel.setMethodCallHandler(FlutterAdmobPlugin(registrar.context()))

      val interstitialChannel = MethodChannel(registrar.messenger(), "admob/interstitial")
      interstitialChannel.setMethodCallHandler(AdmobInterstitial(registrar))

      val rewardChannel = MethodChannel(registrar.messenger(), "admob/reward")
      rewardChannel.setMethodCallHandler(AdmobReward(registrar))

      registrar
        .platformViewRegistry()
        .registerViewFactory("admob/banner", AdmobBannerFactory(registrar.messenger()))
    }
  }
  ...
  override fun onMethodCall(call: MethodCall, result: Result) {
    when(call.method) {
      ...
      "initAdmob" -> MobileAds.initialize(context, call.arguments())
